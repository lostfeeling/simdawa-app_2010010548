...
class ProdiModel extends CI_Model
{
    ...
    public function update_prodi()
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('pesan', "Data Prodi berhasil di ubah!");
            $this->session->set_flashdata('status', true);
        } else {
            $this->session->set_flashdata('pesan', "Data Prodi gagal di ubah!");
            $this->session->set_flashdata('status', false);
        }
    }
    ...
    public function delete_prodi($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            $this->session->set_flashdata('pesan', "Data Prodi berhasil di hapus!");
            $this->session->set_flashdata('status', true);
        } else {
            $this->session->set_flashdata('pesan', "Data Prodi gagal di hapus!");
            $this->session->set_flashdata('status', false);
        }
    }
}
