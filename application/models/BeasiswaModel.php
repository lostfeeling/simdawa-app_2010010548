...
class BeasiswaModel extends CI_Model
{
    ...
    public function insert_beasiswa()
    {
        $data = [
            ...
            'tanggal mulai' => $this->input->post('tanggal_mulai'),
            'tanggal selesai' => $this->input->post('tanggal_selesai'),
            'jenis_id' => $this->input->post('jenis_id')
        ];
        $this->db->insert($this->tabel, $data);
        if ($this->db->affected_row() > 0) {
            $this->session->set_flashdata('pesan', "Data basiswa berhasil ditambahkan!");
            $this->session->set_flashdata('status', true);
        } else {
            $this->session->set_flashdata('pesan', "Data basiswa gagal ditambahkan!");
